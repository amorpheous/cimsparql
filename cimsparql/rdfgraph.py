...
class RdflibGraph(CimModel):
    def __init__(
            self, mapper: CimModel = None): #, infer: bool = False, sameas: bool = True
    #) -> None:
        """GraphDB client
        ...
        graph = Graph()
        graph.parse("/home/frsi/abot/grid-data/EQ/PE2.20210107T140624Z.EQ.xml")
        self.sparql = graph

    #def __str__(self) -> str:
     #   return f"<GraphDBClient object, service: {self.service}>"

    def _setup_client(self): #, infer: bool, sameas: bool, **kwargs) -> None:
       ...
       # self.sparql = graph #SPARQLWrapper(self.service)
       ...
       # self.prefixes = 'svk' #self._service
...
#            self._service = service
    ...
    def _get_table(self, query: str, limit: int) -> Tuple[pd.DataFrame, Dict]:
        ...
        processed_results = self.sparql.query(self._query_with_header(query, limit))
